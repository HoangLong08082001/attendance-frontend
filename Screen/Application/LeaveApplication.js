...
const LeaveScreen = (props) => {
  ...
  const handleSubmit = () => {
    try {
      ...
      let to =
        toTime && toMinute
          ? (toTime + ":" + toMinute)?.toString()
          : checkout_setting;

      if (resumeId) {
        if (selectedOption === "full-day") {
          updateApplication1(resumeId, start, start, reason);
        ...
        if (selectedOption === "by-day") {
          if (endDate >= startDate) {
            updateApplication1(resumeId, start, end, reason);
        ...
        if (selectedOption === "by-hour") {
          if (to > from && endDate >= startDate) {
            updateApplication15(start, end, reason, from, to);
        ...
        if (current_resume < limit_resume_year) {
          if (selectedOption === "full-day") {
            if (start && reason) {
              handleInsertResume1(start, start, reason);
              setStartDate(new Date());
              setEndDate(new Date());
              setFromMinute(null);
              setFromTime(null);
          ...
          if (selectedOption === "by-day") {
            if (reason && start && end) {
              if (endDate >= startDate) {
                handleInsertResume1(start, end, reason);
                setStartDate(new Date());
                setEndDate(new Date());
                setFromMinute(null);
                setFromTime(null);
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={styles.container}>
          ...
          {limit === true && (
            <DialogComponent
              isVisible={limit}
              message={"Bạn đã hết số lượng phép năm"}
              onCancel={() => setLimit(false)}
              title="Cảnh báo"
              isCancel={true}
              lotties={2}
              cancelText="OK"
